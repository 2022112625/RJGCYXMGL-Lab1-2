...
public interface Graph<L> {
    ...
    public static <L> Graph<L> empty() {
        return new Graph<L>() {
            private Set<L> vertices = new HashSet<>(); // 顶点集合

            @Override
            public boolean add(L vertex) {
                return vertices.add(vertex); // 向顶点集合中添加顶点
            }

            @Override
            public int set(L source, L target, int weight) {
                throw new UnsupportedOperationException("set method is not supported in an empty graph");
            }

            @Override
            public boolean remove(L vertex) {
                return vertices.remove(vertex); // 从顶点集合中移除顶点
            }

            @Override
            public Set<L> vertices() {
                return new HashSet<>(vertices); // 返回顶点集合的副本，以防止修改原集合
            }

            @Override
            public Map<L, Integer> sources(L target) {
                return Collections.emptyMap(); // 返回空的源顶点到目标顶点的映射
            }

            @Override
            public Map<L, Integer> targets(L source) {
                return Collections.emptyMap(); // 返回空的目标顶点到源顶点的映射
            }
        };
    }
